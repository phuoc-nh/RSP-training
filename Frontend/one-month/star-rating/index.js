...
function stars(root, max, value) {
	// const root = document.getElementById('star-rating')
	const STAR_ICON_CLASS = 'star-icon';
	const STAR_ICON_FILLED_CLASS = 'star-icon-filled';
	const STAR_TEMPLATE = `<svg
     xmlns="http:www.w3.org/2000/svg"
	...
	root.innerHTML = stars

	// add event listener

	root.addEventListener('mouseover', (event) => {
		const el = event.target.closest(`.${STAR_ICON_CLASS}`)
		console.log('el', el)
		if (!el) {
			return
		}
	...
	// color stars up to current value
	...
	function highlight(id) {
		...
		Array.from(elements).forEach((el, index) => {
			if (index <= id) {
				el.classList.add(STAR_ICON_FILLED_CLASS)
			} else {
				el.classList.remove(STAR_ICON_FILLED_CLASS)
			}
		})
...
stars(document.getElementById('star-rating'), 5, 2)
stars(document.getElementById('star-rating-2'), 10, 5)